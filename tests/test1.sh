    ...
    for e in $EVENT_RANGE; do
        for cn in $CONN_RANGE; do
            # for b in $BUFF_LENN; do
                ...
                echo "Running: Threads=$t, MaxEvents=$e"

                # Run the combined executable in background
                ../build/concurrent_server --threads $t --maxperthrd $e &
                CPP_PID=$!
                sleep 1
                ...
                echo "wrk -t11 -c$cn -d20s --latency http://localhost:8080/"
                wrk -t11 -c$cn -d20s --latency http://localhost:8080/
